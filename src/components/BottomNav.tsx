...
interface BottomNavProps {
  /** Optional quick-add action. Omitted on LMS pages where the plan is generated automatically. */
  onAddClick?: () => void;
}
...
export function BottomNav({ onAddClick }: BottomNavProps = {}) {
  ...
  const navItems = [
    ...
    { to: "/settings" as const, icon: Settings, label: "Pengaturan" },
  ];
        ...
        <nav className="relative flex items-center rounded-2xl border border-border/60 bg-card/80 backdrop-blur-2xl px-1.5 py-1.5 shadow-[0_-4px_24px_-4px_rgba(0,0,0,0.08)]">
          ...
          {activeIndex >= 0 && (
            <div
              ...
              style={{
                width: `calc((100% - ${12 + (onAddClick ? 48 : 0)}px) / 2)`,
                left: `calc(6px + ${activeIndex} * ((100% - ${12 + (onAddClick ? 48 : 0)}px) / 2))`,
              }}
            />
          )}
